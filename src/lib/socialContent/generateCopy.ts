...
4. Tono: directo, con autoridad — no tibio. Priorizá que la pieza genere reacción/atención/comentarios por sobre sonar siempre prolijo o corporativo: está bien ser polémico, picante, hasta un poco provocador si el tema lo pide (mito_polemico, confesion_incomoda, error_viral, pregunta_incomoda existen justamente para esto). Lo que NO cambia es la regla 1 (nada de frases genéricas) y la regla 5 de abajo (nunca a costa de mentir o poner en riesgo a alguien). Emojis: máximo 3-4 en todo el caption de Instagram, usados con intención, no decorativos al final de cada línea.
5. LÍNEA ROJA NO NEGOCIABLE (esto sí es fijo, sin excepción por más picante que sea el ángulo): nunca prometas resultados irreales ("perdé 5kg en una semana") ni des indicaciones médicas específicas (dosis, medicamentos, patologías individuales), ni inventes estadísticas/estudios que no existen, ni ataques o nombres a una persona/marca puntual — la provocación va contra ideas/creencias populares, no contra individuos. Hablá en términos generales y basados en evidencia real.
...
const TOPIC_INSTRUCTIONS: Record<SocialTopic, string> = {
  ...
  confesion_incomoda: `Contá algo que la industria fitness (suplementos, gyms, planes genéricos, "influencers" en general sin nombrar a nadie puntual) preferiría que la gente no supiera — un incentivo económico o de negocio que explica por qué te venden cierto consejo aunque no sea lo mejor para vos (ej: por qué los gyms venden rutinas genéricas en vez de personalizadas, por qué "más suplementos" siempre es la respuesta fácil de vender, por qué el conteo de calorías de las apps populares está mal calibrado a propósito para simplificar). Tono de "te cuento la posta que no te cuentan", con el mecanismo real detrás, no paranoia sin sustento.`,
  mito_polemico: `Versión más filosa de un mito fitness: elegí una creencia MUY popular (la que repiten cuentas grandes de fitness todo el tiempo) y desarmala con evidencia, sin miedo a decir directamente "esto que ves en todos lados está mal". Ideas: "el mito de que hay que entrenar en ayunas para quemar más grasa", "la obsesión con el número de la balanza en vez de composición corporal", "por qué el 'no pain no gain' te está lesionando", "la mentira del detox/limpieza". Tono con filo, casi de discusión, pero siempre anclado en el mecanismo real — cero insultos, cero ataques a personas puntuales.`,
  comparacion_shock: `Una comparación numérica o visual que genere un "¿¿QUÉ??" — algo que la gente no se espera y quiere compartir. Ideas: cuánto tenés que caminar/correr para "gastar" tal alimento típico, cuánta azúcar/sodio oculto tiene algo que la gente considera "sano", cuánto tiempo real se tarda en ver resultados vs. lo que promete la industria, comparar el gasto calórico real de dos actividades que la gente cree equivalentes y no lo son. El shock tiene que venir de un dato REAL y verificable, no exagerado ni inventado.`,
  error_viral: `Un error MUY común que ves todo el tiempo (en el gym, en redes, en la gente que arranca) que casi nadie corrige, con tono de urgencia/indignación genuina ("no puedo creer que todavía nadie te haya dicho esto"). Tiene que ser específico y accionable, no una queja vaga. Ideas: el error de técnica más común en un ejercicio popular, el error de armar el plato "saludable" que sabotea el objetivo sin que la persona lo note, el error de progresión que estanca a la gente en el gym durante meses.`,
  pregunta_incomoda: `Abrí con una pregunta directa e incómoda que haga que la persona se sienta aludida en 3 segundos — el objetivo es que pare de scrollear porque se sintió atacada (con cariño) por la pregunta. Ideas de ángulo: cuestionar una excusa común ("¿hace cuánto que decís que empezás el lunes?"), cuestionar una prioridad ("¿por qué le dedicás más tiempo a elegir qué serie ver que a planear qué vas a comer?"), cuestionar una comparación tóxica con otros. Después de la pregunta, dale valor real, no la dejes solo en el golpe emocional.`,
};
