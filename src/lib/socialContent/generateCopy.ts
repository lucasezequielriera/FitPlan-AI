...
const TOPIC_INSTRUCTIONS: Record<SocialTopic, string> = {
  ...
  tecnica_correcta_incorrecta: `Contrasta DIRECTAMENTE la forma correcta y la incorrecta de UN ejercicio popular (rota entre sentadilla, peso muerto, press banca, dominada, plancha, remo — no uses siempre el mismo). Describe primero el error tal como se ve de verdad (qué articulación compensa, qué rango se recorta, qué parte del cuerpo hace trampa) y después la versión correcta con la señal concreta para sentirla bien (qué debería notar la persona, qué punto de referencia usar). El contraste tiene que ser tan nítido que se entienda sin ver el video, aunque está pensado para acompañarse de demostración visual.`,
  mito_suplemento: `Desarma un mito sobre UN suplemento concreto (rota entre creatina, proteína en polvo, quemadores de grasa, BCAAs, pre-entreno, multivitamínicos — no uses siempre el mismo). Ideas: la creatina no retiene grasa ni daña el riñón en persona sana, la proteína en polvo no es "química" sino el mismo macronutriente en polvo, los quemadores de grasa no sustituyen el déficit calórico, los BCAAs son redundantes si ya comés suficiente proteína completa. Explica el mecanismo real, sin prometer ni desprestigiar el suplemento en sí — el filo va contra el mito, no contra el producto.`,
  ...
  variantes_ejercicio: `Compara DOS variantes de un mismo movimiento (rota entre sentadilla libre vs. goblet/hack, press banca vs. press inclinado/mancuernas, dominada vs. jalón, peso muerto convencional vs. rumano, zancada vs. búlgara — no uses siempre el mismo par) y explica CUÁNDO conviene cada una: qué músculo enfatiza más una que la otra, qué articulación protege o exige más, y para qué objetivo o limitación (movilidad, lesión previa, nivel) tiene sentido elegir una sobre la otra. Nada de "las dos sirven": el valor está en el criterio concreto para decidir.`,
